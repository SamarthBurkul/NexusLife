    ...
    <div className="relative">
      ...
      <div className="space-y-12">
        {events && events.length > 0 ? (
          events.map((event, i) => {
            const eventType = event.event_type || event.type || 'education';
            const config = typeConfig[eventType] || typeConfig.education;
            const isLeft = i % 2 === 0;
                ...
                <div className={`w-5/12 ${isLeft ? 'text-right' : 'text-left'}`}>
                  <div className="bg-card border border-gray-800 rounded-xl p-4 inline-block hover:border-gray-700 transition">
                    <div className={`flex items-center gap-2 mb-1 ${isLeft ? 'justify-end' : 'justify-start'}`}>
                      <span className={`text-xs font-medium ${config.textColor}`}>{eventType.charAt(0).toUpperCase() + eventType.slice(1)}</span>
          ...
          <div className="text-center text-gray-400 py-12">No events yet. Add your first life milestone!</div>
        )}
      </div>
    </div>
  );
